...
class SoftObject:
    def __init__(self, shape, seed_size):
        ...
        self.jacobian_y = ti.Vector.field(2*self.PARTICLE_NUM, dtype=ti.f64, shape=2)
        self.J = ti.field(ti.f64, shape=(2, 2))             # Jacobian matrix
    ...
    def diff_pd(self, par_L:ti.types.ndarray(), itr_num:ti.i32):
        """
        ...
        # print(self.grad_y.to_numpy()[10])
        # np.savetxt('grad_y.csv', self.grad_y.to_numpy())

        return z_np


    @ti.kernel
    def grad_loss(self, z:ti.types.ndarray()):
        """
        Sum the gradient of the loss respect to y
        :return:
        """
        for i in range(self.PARTICLE_NUM):
            idx0, idx1 = i*self.dim, i*self.dim+1
            self.grad_y[i].x = z[idx0]*self.node_mass[i]/self.dt**2
            self.grad_y[i].y = z[idx1]*self.node_mass[i]/self.dt**2


    # @ti.kernel
    def grad_jacobian(self, itr_num:ti.i32):
        """
        Calculate the Jacobian matrix by gradient method
        :return:
        """
        for marker_i in self.dmarker:
            dmarker_tmp_np = self.dmarker[marker_i].to_numpy()
            z_np = self.diff_pd(dmarker_tmp_np, itr_num)

            for i in range(self.PARTICLE_NUM):
                idx0, idx1 = i * self.dim, i * self.dim + 1
                self.jacobian_y[marker_i][idx0] = z_np[idx0] * self.node_mass[i] / self.dt ** 2
                self.jacobian_y[marker_i][idx1] = z_np[idx1] * self.node_mass[i] / self.dt ** 2

            grasp_idx = self.grasp_particle_list[0]
            self.J[marker_i, 0] = self.jacobian_y[marker_i][grasp_idx*self.dim]
            self.J[marker_i, 1] = self.jacobian_y[marker_i][grasp_idx*self.dim+1]
    ...
    def substep(self, step_num):
        ...
        z_np = self.diff_pd(self.dL.to_numpy(), 10)
        self.grad_loss(z_np)
        # self.diff_data()

        # Calculate the Jacobian matrix
        self.grad_jacobian(10)
    ...
    def control_grasp(self, learning_rate):
        ...
        return self.GRASP_VEL[0]*self.dt
