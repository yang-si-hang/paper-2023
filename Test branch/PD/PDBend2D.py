...
class SoftBend2D:
    ...
    def rhs_shear(self):
        ...
        for f_i in range(self.ELEMENT_N):
            ...
            for ele_idx, dim_idx in ti.ndrange(3, 3):
                idx = self.ele[f_i][dim_idx]
                self.rhs[idx+dim_idx] += F_ATBp[ele_idx, dim_idx]
...
def main():
    ...
    # soft.init_vel()
    np.savetxt("edge.csv", soft.edge.to_numpy(), fmt='%d', delimiter=",")
