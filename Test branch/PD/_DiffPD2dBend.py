    ...
    def construct_rhs_bend(self):
        ...
        for e in self.mesh.edges:
            if e.border == False:
                ...
                v1, v2 = e.verts[0], e.verts[1]
                v1_id, v2_id, v3_1_id, v3_2_id = e.neigh_verts[0], e.neigh_verts[1], e.neigh_verts[2], e.neigh_verts[3]
                v3_1_pos, v3_2_pos = self.mesh.verts.pos[v3_1_id], self.mesh.verts.pos[v3_2_id]

                v_f = e.cot_w @ ti.Matrix.rows([v1.pos, v2.pos, v3_1_pos, v3_2_pos]) / e.voronoi_sqrt # dim: 3*1
                e.v_f = v_f

                if v_f.norm() > 0.:
                    self.Bp_bend[e.id] = e.n * v_f / v_f.norm()
                else:
                    normal = ti.Matrix.rows([(v1.pos - v2.pos).cross(v3_1_pos - v2.pos).normalized()])
                    if v_f_old.norm() > 0.:
...
def main():
    ...
    soft.marker_pos_desired[0] = soft.mesh.verts.pos[soft.marker_idx] + ti.Vector([0., 0., 0.01])
