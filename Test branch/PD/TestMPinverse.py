...
def partial():
    ...
    dPP_dsig_vec = my_solve(PP_coef_A, PP_coef_B)
    """ 
    ...
    for row_idx, col_idx in ti.ndrange(1, 1):
        ...
        if ti.abs(s1-s2) < 1.e-6:
            ...
            sol = ti.solve(coef_A, coef_B, ti.f64)
            # 或者用系数求和
        ...
        if ti.abs(s1 -s3) < 1.e-6:
            ...
            coef_A = ti.Matrix([[s3, s1], [s1, s3]])
            coef_B = ti.Vector([U[row_idx, 0]*V[col_idx, 2], -U[row_idx, 2]*V[col_idx, 0]])
            sol = ti.solve(coef_A, coef_B, ti.f64)
        ...
        if ti.abs(s2 - s3) < 1.e-6:
            ...
            coef_A = ti.Matrix([[s3, s2], [s2, s3]])
            coef_B = ti.Vector([U[row_idx, 1]*V[col_idx, 2], -U[row_idx, 2]*V[col_idx, 1]])
            sol = ti.solve(coef_A, coef_B, ti.f64)
            # print('sol', sol)
        ...
        dPP_df = ti.Matrix.zero(ti.f64, dim, dim)
