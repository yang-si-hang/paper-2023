...
loss = np.loadtxt('loss4.csv', delimiter=',')
grasp_pos = np.loadtxt('grasp_pos4.csv', delimiter=',')
marker_pos = np.loadtxt('marker_pos4.csv', delimiter=',')

fig, axs = plt.subplots(3, 1, figsize=(10, 12), dpi=100)
plt.rcParams['font.family'] = 'Times New Roman'  # 设置刻度标签的字体

# Plot 1: Loss
axs[0].plot(loss, linewidth=2)
...
axs[1].plot(grasp_pos[:, 0], -grasp_pos[:, 1], linewidth=2)
...
axs[2].plot(marker_pos[:, 0], -marker_pos[:, 1], linewidth=2)
axs[2].plot(0.09, 0.02, marker='o', markersize=10, color="black")
axs[2].plot(0.09+0.002, 0.02, marker='o', markersize=10, color="red")
