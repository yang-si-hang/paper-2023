"""
This file implements the Projective Dynamics simulation in 2D. The position constraint is
apllied to the marker points.
"""
...
GRASP_VEL = ti.Vector([0.020918, 0.013936])/5.
...
def init_pos():
    ...
    for i, j in ti.ndrange(N + 1, W + 1):
        k = i*(W+1)+j
        pos[k] = ti.Vector([i/N*LL, j/W*WW]) + ti.Vector([0.0, -WW/2])
        pos_init[k] = pos[k]
        vel[k] = ti.Vector([0, 0])
        if i == 0:
            boundary_labels[k] = 1
...
def marker_nearest_paticle(marker_point_np:ti.types.ndarray()):
    N = marker_point_np.shape[0]
    marker_point = ti.Vector.field(2, dtype=ti.f64, shape=N)
    marker_point.from_numpy(marker_point_np)
    marker_nearest = ti.ndarray(dtype=ti.i32, shape=N)
    ...
    for i in range(N):
        ...
        marker_nearest[i] = nearest_idx

    return marker_nearest.to_numpy()
