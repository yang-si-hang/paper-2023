...
在原来的基础上将旋转部分的状态量表示为角度,而不是四元数
created at 2024-08-10 by hsy
"""
...
def generate_geometric(length, particle_num:int):
    ...
    X = length * np.cos(0 * np.pi / 6)
    Y = length * np.sin(0 * np.pi / 6)
    ...
    element_angle_np = np.zeros(particle_num-1, dtype=np.float64)
    for idx in range(particle_num-1):
        element_angle_np[idx] = np.arctan2(node_np[idx+1][1] - node_np[idx][1], node_np[idx+1][0] - node_np[idx][0])

    return node_np, element_np, element_angle_np
...
class PD1D:
    def __init__(self, length, radius, seed_size:float):
        ...
        # self.contact_element_weight = 1.e5
        ...
        node_np, element_np, element_angle_np = generate_geometric(self.length, self.PARTICLE_NUM)
        np.savetxt('node_pos_init.csv', node_np, delimiter=',', fmt='%.6f')
        np.savetxt('element.csv', element_np, delimiter=',', fmt='%d')
        np.savetxt('element_angle.csv', element_angle_np, delimiter=',', fmt='%.6f')
        ...
        self.element_inertia = ti.field(dtype=ti.f64, shape=self.ELEMENT_NUM)
        self.element_angle = ti.field(dtype=ti.f64, shape=self.ELEMENT_NUM)
        self.element_angle_init = ti.field(dtype=ti.f64, shape=self.ELEMENT_NUM)
        self.element_angle_new = ti.field(dtype=ti.f64, shape=self.ELEMENT_NUM)
        self.element_angle_delta = ti.field(dtype=ti.f64, shape=self.ELEMENT_NUM)
        self.element_sn = ti.field(dtype=ti.f64, shape=self.ELEMENT_NUM)
        self.element.from_numpy(element_np, dtype=np.int32)
        self.element_angle.from_numpy(element_angle_np, dtype=np.float64)
        self.element_angle_init.from_numpy(element_angle_np, dtype=np.float64)
        self.element_angle_delta.from_numpy(np.zeros((self.ELEMENT_NUM,), dtype=np.float64))
        ...
        self.A_stretch = ti.Matrix.field(3, 5, dtype=ti.f64, shape=())
        self.A_bend = ti.Matrix.field(2, 2, dtype=ti.f64, shape=())
        self.Bp_stretch = ti.Vector.field(3, dtype=ti.f64, shape=self.ELEMENT_NUM)
        self.Bp_bend = ti.Vector.field(2, dtype=ti.f64, shape=self.ANGLE_NUM)
        self.lhs = ti.field(dtype=ti.f64, shape=(self.PARTICLE_NUM*self.dim+self.ELEMENT_NUM, self.PARTICLE_NUM*self.dim+self.ELEMENT_NUM))
        self.rhs = ti.field(dtype=ti.f64, shape=self.PARTICLE_NUM*self.dim+self.ELEMENT_NUM)
        self.lhs.fill(0.)

        self.stretch_constraint = ti.field(dtype=ti.f64, shape=(self.PARTICLE_NUM*self.dim+self.ELEMENT_NUM, self.PARTICLE_NUM*self.dim+self.ELEMENT_NUM))
        self.bend_constraint = ti.field(dtype=ti.f64, shape=(self.PARTICLE_NUM*self.dim+self.ELEMENT_NUM, self.PARTICLE_NUM*2+self.ELEMENT_NUM))

        self.dBp_stretch_dqu = ti.Matrix.field(3, 5, dtype=ti.f64, shape=self.ELEMENT_NUM)
        self.dBp_bend_du = ti.Matrix.field(2, 2, dtype=ti.f64, shape=self.ANGLE_NUM)
        self.AT_dBp_stretch_dqu = ti.Matrix.field(5, 5, dtype=ti.f64, shape=self.ELEMENT_NUM)
        self.AT_dBp_bend_du = ti.Matrix.field(2, 2, dtype=ti.f64, shape=self.ANGLE_NUM)
        self.rhs_dA = ti.field(dtype=ti.f64, shape=(self.PARTICLE_NUM*self.dim+self.ELEMENT_NUM, self.PARTICLE_NUM*self.dim+self.ELEMENT_NUM))
        self.z = ti.field(dtype=ti.f64, shape=(self.PARTICLE_NUM*self.dim+self.ELEMENT_NUM))
        self.dqu_const = ti.field(dtype=ti.f64, shape=self.PARTICLE_NUM*self.dim+self.ELEMENT_NUM)
        self.dL_dy = ti.field(dtype=ti.f64, shape=self.PARTICLE_NUM*self.dim + self.ELEMENT_NUM)
        self.grad_diffdata = ti.field(dtype=ti.f64, shape=self.PARTICLE_NUM*self.dim + self.ELEMENT_NUM)

        # Finite difference for validation
        # self.delta = ti.cast(1e-6, ti.f64)
        self.grad_finite = ti.field(dtype=ti.f64, shape=self.PARTICLE_NUM*self.dim + self.ELEMENT_NUM)
        ...
        self.contact_vel = ti.Vector.field(2, dtype=ti.f64, shape=self.contact_par_num)
        ...
        self.contact_vel[0] = ti.Vector([0., 0.]) / self.dt

        self.node_desired_pos = ti.Vector.field(2, dtype=ti.f64, shape=self.contact_par_num)
        self.element_desired_angle = ti.field(dtype=ti.f64, shape=self.contact_ele_num)
    ...
    def construct_mass(self):
        ...
        for ele_idx in range(self.ELEMENT_NUM):
            self.element_inertia[ele_idx] = self.l * self.rho * J1


    def construct_weight(self):
        self.stretch_weight = self.E * self.section_area * self.l
        self.bend_weight = 2 * 2 * self.G * tm.pi * self.radius ** 4 / self.l
    ...
    def local_solve(self):
        ...
        for angle_idx in range(self.ANGLE_NUM):
            ...
            cos_theta = u1.dot(u2)
            u_average_unit = (u1 + u2) / ti.sqrt((1 + cos_theta) / 2) / 2
    ...
    def construct_rhs(self):
        ...
        for angle_idx in range(self.ANGLE_NUM):
            idx1, idx2 = self.PARTICLE_NUM + angle_idx, self.PARTICLE_NUM + angle_idx + 1
            u_idx_vec = ti.Vector([idx1*2, idx1*2+1, idx2*2, idx2*2+1])
            A_i = self.A_bend[None]
    ...
    @ti.kernel
    def init_vel(self):
        self.node_vel[0][0] = -100.
        # self.node_force[0][0] = 9.8 * self.node_mass[0]
...
def main():
    ...
    soft_obj = MyObj(length=1., radius=0.01, seed_size=0.4)
