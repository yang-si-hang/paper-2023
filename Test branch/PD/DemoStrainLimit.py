"""
This file is built based on demo.py, changed the manifold constraint to strain limit in local
solver.
Volume constraint are not used in this file by m_weight_volume = 0.0.
"""
...
sigma_x_limit, sigma_y_limit = [0.99, 1.5], [0.9, 1.1]
...
print("m_weight_strain/volume", m_weight_strain/volume, "  "
      "m_weight_volume/volume", m_weight_volume/volume)
...
def local_solve_build_bp_for_all_constraints():
    for i in range(NF):
        ...
        # Add strain limit constraint
        for j in range(2):
            if sigma[j,j] < sigma_x_limit[0]:
                sigma[j,j] = sigma_x_limit[0]
            elif sigma[j,j] > sigma_x_limit[1]:
                sigma[j,j] = sigma_x_limit[1]
        Bp[i] = U @ sigma @ V.transpose()
