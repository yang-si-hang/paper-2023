...
class SoftBend2D:
    def __init__(self, shape, E:float, nu:float, dt:float, density:float, g=-9.8):
        ...
        if isinstance(self.shape, str):
            ...
            node_np, edge_np, ele_np = mesh_obj_tri(self.shape, 0.025)
        ...
        self.bend_energy = ti.field(dtype=ti.f64, shape=self.PARTICLE_N)
        self.bp_sig = ti.Vector.field(2, dtype=ti.f64, shape=self.ELEMENT_N)
        ...
        self.v_f = ti.Vector.field(3, dtype=ti.f64, shape=self.PARTICLE_N)

        self.fix_particle_list = list(range(5))
        # self.contact_particle_list = list(range(420, 441))
        self.contact_particle_list = list(range(20, 25)) #[5, 10, 15, 9, 14, 19]
    ...
    def construct_mass(self):
        for f_i in range(self.ELEMENT_N):
            ...
            self.stretch_lim_weight[f_i] = 0.e3 * self.stretch_weight[f_i]
    ...
    def construct_cotangent(self):
        ...
        for q in self.mesh.verts:
            ...
            q.bend_weight = self.bend_weight * q.voronoi
            for e in q.edges:
                if e.border == True:
                    q.border = True         # 边界点,没有曲率
    ...
    def construct_lhs_bend(self):
        ti.mesh_local(self.mesh.verts.bend_weight, self.mesh.verts.v_g, self.mesh.verts.voronoi, self.mesh.edges.cotagent_w)
        for q in self.mesh.verts:
            q_id = q.id
            ...
            if q.border == False:
                # print(f"Vertex {q_id}: {q.neighbor_num} neighbors")
                for e in q.edges:
                ...
                for e_m in q.edges:
                    ...
                    for e_n in q.edges:
                        ...
                        self.lhs[q_m.id, q_n.id] += bend_weight * cot_w_mul / q.voronoi**2
                        self.lhs_bend_ti[q_m.id, q_n.id] += bend_weight * cot_w_mul / q.voronoi**2
                        # print(f"LHS bend: {q_m.id}--{q_n.id}: {cot_w_mul}")
                ...
                for e in q.edges:
                    ...
                    q.v_g += e.cotagent_w / q.voronoi * (q_neighbor.pos - q.pos)

                    self.lhs[q_id, q_neighbor.id] += -cot_w_sum * e.cotagent_w / q.voronoi**2 * bend_weight
                    self.lhs[q_neighbor.id, q_id] += -cot_w_sum * e.cotagent_w / q.voronoi**2 * bend_weight
                    self.lhs_bend_ti[q_id, q_neighbor.id] += -cot_w_sum * e.cotagent_w / q.voronoi**2 * bend_weight
                    self.lhs_bend_ti[q_neighbor.id, q_id] += -cot_w_sum * e.cotagent_w / q.voronoi**2 * bend_weight
                    # print(f"LHS bend: {q_id}--{q_neighbor.id}: {-cot_w_sum * e.cotagent_w}")
                    # print(f"LHS bend: {q_neighbor.id}--{q_id}: {-cot_w_sum * e.cotagent_w}")

                self.lhs[q_id, q_id] += cot_w_sum**2 / q.voronoi**2 * bend_weight
                self.lhs_bend_ti[q_id, q_id] += cot_w_sum**2 / q.voronoi**2 * bend_weight
    ...
    def construct_rhs_stretch(self):
        ...
        for f_i in range(self.ELEMENT_N):
            ...
            # print(f"Face {f_i}: F_ATBp_lim: {F_ATBp_lim}")
            # print(f"Bp_shear_lim_i: {Bp_shear_lim_i}")
            # print(f"F_AT: {F_AT}")
    ...
    def construct_rhs_bend(self):
        ti.mesh_local(self.mesh.verts.bend_weight, self.mesh.verts.v_g, self.mesh.verts.pos, self.mesh.verts.voronoi, self.mesh.edges.cotagent_w)
        for q in self.mesh.verts:
            q_id = q.id
            bend_weight = q.bend_weight
            cot_w_sum = 0.
            ...
            if q.border == False:
                ...
                for e in q.edges:
                    ...
                    v_f += e.cotagent_w / q.voronoi * (q_neighbor.pos - q.pos)
                    cot_w_sum += e.cotagent_w
                self.v_f[q_id] = v_f
                # print(f"v_f: {v_f}")

                if v_f.norm() == 0.:
                    self.Bp_bend[q_id] = q.v_g
                else:
                    self.Bp_bend[q_id] = q.v_g.norm() * v_f / v_f.norm()
                Bp_bend_i = self.Bp_bend[q_id]

                self.bend_energy[q_id] = 0.5 * bend_weight * (v_f - Bp_bend_i).norm()**2
                ...
                for e in q.edges:
                    ...
                    for dim_idx in range(3):
                        self.rhs[q_neighbor.id*3+dim_idx] += e.cotagent_w / q.voronoi * Bp_bend_i[dim_idx] * bend_weight
                        self.rhs_bend[q_neighbor.id*3+dim_idx] += e.cotagent_w / q.voronoi * Bp_bend_i[dim_idx] * bend_weight
                    # print(f"RHS bend: {q_neighbor.id*3}--{q_neighbor.id*3+2}: {e.cotagent_w * Bp_bend_i[0] * bend_weight}; {e.cotagent_w * Bp_bend_i[1] * bend_weight}; {e.cotagent_w * Bp_bend_i[2] * bend_weight}")
                
                for dim_idx in range(3):
                    self.rhs[q_id*3+dim_idx] += -cot_w_sum / q.voronoi * Bp_bend_i[dim_idx] * bend_weight
                    self.rhs_bend[q_id*3+dim_idx] += -cot_w_sum / q.voronoi * Bp_bend_i[dim_idx] * bend_weight
                    # print(f"RHS bend: {q_id*3+dim_idx}: {-cot_w_sum * Bp_bend_i[dim_idx] * bend_weight}")
    ...
    def construct_rhs_poistional(self):
        ...
        for i in range(self.CON_N):
            q_i = self.contact_particle_ti[i]
            self.rhs[q_i*3] += self.positional_weight * (self.node_pos_init[q_i].x + self.contact_vel[i].x * self.dt)
            self.rhs[q_i*3+1] += self.positional_weight * (self.node_pos_init[q_i].y + self.contact_vel[i].y * self.dt)
            self.rhs[q_i*3+2] += self.positional_weight * (self.node_pos_init[q_i].z + self.contact_vel[i].z * self.dt)
    ...
    @ti.kernel
    def cal_v_f(self):
        ti.mesh_local(self.mesh.verts.bend_weight, self.mesh.verts.v_g, self.mesh.verts.pos, self.mesh.edges.cotagent_w)
        for q in self.mesh.verts:
            q_id = q.id
            cot_w_sum = 0.
            self.Bp_bend[q_id] = ti.Vector.zero(ti.f64, 3)
            ...
            if q.border == False:
                ...
                for e in q.edges:
                    ...
                    v_f += e.cotagent_w / q.voronoi * (q_neighbor.pos - q.pos)
                    print(f"pos diff {q_neighbor.id}-{q_id}: {q_neighbor.pos - q.pos}; cotangent weight: {e.cotagent_w/q.voronoi}")
                    cot_w_sum += e.cotagent_w
                self.v_f[q_id] = v_f
...
def main():
    class Soft(SoftBend2D):
        def __init__(self, shape:list, E:float, nu:float, dt:float, density:float, g=-0.1):
            super().__init__(shape, E, nu, dt, density, g)
    
    soft = Soft([0.1, 0.1], 1.e3, 0.4, 0.01, 10e2)
    # soft.preset_gui([0.05, 0.1, 0.3], [0.05, 0.1, 0.], [0., 1., 0.])
    soft.preset_gui([-0.2, 0.05, 0.15], [0.05, 0.1, 0.], [0., 0., 1.])

    q_border = soft.mesh.verts.border.to_numpy()
    q_curvature_indices = np.where(q_border == 0)[0].tolist()
    ...
    for itr in range(20):
        ...
        print(f"Vertex normal: \n{np.hstack((soft.v_f.to_numpy()[q_curvature_indices], np.linalg.norm(soft.v_f.to_numpy()[q_curvature_indices], axis=1).reshape(-1,1)))}")
        ...
        # time.sleep(0.1)
        print(f"Node 1: {soft.node_pos[1]}")
        print(f"Node 12: {soft.node_pos[12]}")
