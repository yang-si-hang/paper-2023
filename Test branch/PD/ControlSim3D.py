...
class SoftObject:
    ...
    def apply_action(self, action):
        ...
        # vel_trans = np.zeros((self.contact_num, 3), dtype=np.float64)
        # vel_rot = np.zeros((self.contact_num, 3), dtype=np.float64)
        for idx in range(self.contact_num):
            contact_vel[idx] += transl / self.dt
            # vel_trans[idx] = transl / self.dt

        for idx in range(self.contact_num):
            contact_vel[idx] += np.cross(rot, contact_pos[idx] - contact_center) / self.dt
            # vel_rot[idx] = np.cross(rot, contact_pos[idx] - contact_center) / self.dt

        # print(f"Translational movement: \n{vel_trans}")
        # print(f"Rotational movement: \n{vel_rot}")
