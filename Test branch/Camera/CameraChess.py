...
def get_camera_intrinsic():
    ...
    # 内参矩阵
    camera_matrix = np.array([[left_cam_params.fx, 0, left_cam_params.cx],
                              [0, left_cam_params.fy, left_cam_params.cy],
                              [0, 0, 1]], dtype=np.float32)

    # 畸变系数
    dist_coeffs = np.array([left_cam_params.disto[0],
                            left_cam_params.disto[1],
                            left_cam_params.disto[2],
                            left_cam_params.disto[3],
                            left_cam_params.disto[4]], dtype=np.float32)
    ...
    if zed.grab() == sl.ERROR_CODE.SUCCESS:
        # 从相机中检索图像
        zed.retrieve_image(image, sl.VIEW.LEFT)  # 你可以选择 LEFT 或 RIGHT
        ...
        print("Image captured and saved as zed_image.jpg")
