"""
使用liver.msh作为几何网格文件,控制降采样后的节点,以到达期望位置，并记录该节点的位置
...
class MyObject(SoftObject):
    def __init__(self, shape, seed_size, file, marker_list, contact_list):
        super().__init__(shape, seed_size, file, contact_list)
        self.dt = 1. / 100
        self.dots_idx = sorted(set(marker_list) - set(self.fix_particle_list) - set(self.contact_particles_list))
        self.dot_num = len(self.dots_idx)
        self.loss = ti.field(dtype=ti.f64, shape=self.dot_num)
        self.dot_pos = ti.Vector.field(3, dtype=ti.f64, shape=self.dot_num)
        self.dot_pos_init = ti.Vector.field(3, dtype=ti.f64, shape=self.dot_num)
        self.dot_pos_desired = ti.Vector.field(3, dtype=ti.f64, shape=self.dot_num)
        self.dot_error = ti.Vector.field(3, dtype=ti.f64, shape=self.dot_num)
        self.dot_sofa = ti.Vector.field(3, dtype=ti.f64, shape=self.dot_num)
    ...
    def read_desired_pos(self):
        desired_pos = np.loadtxt(script_dir + '/Mesh/node_desired_pos.csv', delimiter=',')
        for i in range(self.dot_num):
            idx = self.dots_idx[i]
            self.dot_pos_init[i] = self.node_init_pos[idx]
            self.dot_pos[i] = self.node_init_pos[idx]
            self.dot_pos_desired[i] = desired_pos[idx]
        print(f"Initial position & Desired position:")
        for i in range(self.dot_num):
            init_pos = ''.join([f"{val:>10.6f}" for val in self.dot_pos_init[i]])
            des_pos = ''.join([f"{val:>10.6f}" for val in self.dot_pos_desired[i]])
            error_val = self.dot_pos_desired[i] - self.dot_pos_init[i]
            error = ''.join([f"{val:>10.6f}" for val in error_val])
            print(f"{self.dots_idx[i]:>3}: [{init_pos}]; [{des_pos}]; [{error}]")
        # print(f"{self.dot_pos_init.to_numpy()}")
        # print(f"Desired position: \n{self.dot_pos_desired.to_numpy()}")


    @ti.kernel
    def update_dot_pos(self):
        for i, idx in ti.static(enumerate(self.dots_idx)):
            self.dot_pos[i] = self.node_pos[idx]


    @ti.kernel
    def construct_L_sofa(self):
        dim = self.dim
        ...
        for i in range(self.dot_num):
            ...
            current_pos = self.dot_sofa[i]
            error_ti = current_pos - desired_pos
            self.dot_error[i] = error_ti
        for i, idx in ti.static(enumerate(self.dots_idx)):
            for d in range(dim):
                self.dL_dq[idx*dim+d] += 2 * self.dot_error[i][d]
    ...
    def compute_gradient(self, dot_sofa:npt.NDArray):
        ...
        self.dot_sofa.from_numpy(dot_sofa)
        self.construct_L_sofa()
        error_np = self.dot_error.to_numpy()
        loss_tmp = np.linalg.norm(error_np, axis=1)**2
        self.loss.from_numpy(loss_tmp)
        self.diff_pd(10)
        self.compute_grad_y()
...
def main():
    contact_idx = [85]
    # marker_idx = [138]
    marker_list = [85, 89, 49, 99, 127, 132, 116, 79, 60, 63, 65, 101, 103, 0, 69, 80, 84, 123, 124, 128, 68, 1, 62, 75,
                   27, 35, 55, 66, 76, 110, 120, 20, 7, 21, 12, 11, 15, 4, 125, 53, 113, 86]
    fix_idx = [3, 39, 64]
    obj_shape = [0.1, 0.02, 0.1]          # 作为一个占位符，此处无用
    learning_rate = 3.e0

    soft_obj = MyObject(obj_shape, 0.01, 'Mesh/liver.msh', marker_list, contact_idx)
    ...
    dots_pos_sofa_init = get_marker_pos(dofs, soft_obj.dots_idx)
    ...
    for step in range(200):
        print(f"Step: {step}---------------------------------------")
        dots_pos_sofa_new = get_marker_pos(dofs, soft_obj.dots_idx)
        ...
        end_movement_np = action_compress(end_speed_np * soft_obj.dt, 6.e-4)

        soft_obj.actuate_action(end_movement_np / soft_obj.dt)
        # print(f'Loss items: {loss_tmp}')
        print(f"Loss sum: {np.sum(loss_tmp)}")
