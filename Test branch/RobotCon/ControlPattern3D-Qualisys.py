...
# pattern中有几个点，重新赋值
POINTS_NUM:int = 4
selected_index = [104, 105, 106, 107]
...
class MyObject(SoftObject):
    def __init__(self, shape:List[float], seed_size:float, mesh_file:str, marker_pos_init, contact_list:List[int], fixed_list:List[int]):
...
async def main():
    ...
    # 需要根据`dots_pos_init`重新排序
    dots_init_pos_soft = np.array([[0.25658509,0.24311180,0.02856585],
                                   [0.18278503,0.24276842,0.02856585],
                                   [0.25491601,0.19220932,0.02856593],
                                   [0.32879595,0.22555393,0.02856588]], dtype=float)
    ...
    soft = MyObject(obj_shape, obj_seed_size, mesh_file, dots_pos_init, contact_list, fixed_list)
    ...
    try:
        for step in range(700):
            ...
            drob = soft.compute_action(contact_list)
            print(f"drob: {-drob}")
            compressed_action = action_compress(-learning_rate*drob, 2.e-3)
            soft.apply_action(compressed_action)

            left_action = compressed_action.tolist()
            print(f"Error: {error_tmp}")
            print(f'Loss items: {loss_tmp}; Loss sum: {np.sum(loss_tmp)}')
            print(f'The tool movement: {left_action}; Length: {np.linalg.norm(left_action)}')
