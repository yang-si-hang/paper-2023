"""
使用PD仿真1D的绳变形,基于Cosserat理论，只考虑拉伸和弯曲,用于控制的带接触点的情况
...
class PD1D:
    def __init__(self, length, radius, seed_size:float):
        ...
        self.dL_dqu = ti.field(dtype=ti.f64, shape=self.PARTICLE_NUM*self.dim+self.ELEMENT_NUM*2)
        ...
        self.marker_pos_desired[0] = self.node_pos_init[self.marker_idx] + ti.Vector([0.01, 0.2])*0.1
    ...
    def precomputation(self):
        ...
        for u_idx in range(self.ELEMENT_NUM):
            for d in ti.static(range(self.dim)):
                self.lhs[(u_idx + self.PARTICLE_NUM) * dim + d, (u_idx + self.PARTICLE_NUM) * dim + d] \
                = self.element_inertia[u_idx][d] / self.dt ** 2
    ...
    def construct_L(self):
        self.dL_dqu.fill(0.)
        # for q_idx in self.contact_particle_list:
        #     self.dL_dqu[(q_idx+7)*self.dim + 0] = 1.
        ...
        L = error.norm()**2
        self.dL_dqu[idx*dim] = 2*(current_pos.x - desired_pos.x)
        self.dL_dqu[idx*dim + 1] = 2*(current_pos.y - desired_pos.y)
        return error, L
    ...
    def construct_dx_const(self):
        ...
        for u_idx in range(self.ELEMENT_NUM):
            ...
            for d in range(2):
                self.dqu_const[(u_idx+self.PARTICLE_NUM)*2+d] = self.element_inertia[u_idx][d] / self.dt ** 2
        ...
        for u_i in ti.static(self.contact_element_list):
            for d in ti.static(range(self.dim)):
                self.dqu_const[(u_i+self.PARTICLE_NUM)*2+d] += self.contact_element_weight
    ...
    def control_grasp(self, learning_rate):
        idx = self.contact_particle_list[0]
        grad_contact = np.array([self.dL_dy[idx*self.dim], self.dL_dy[idx*self.dim+1]])

        self.contact_vel[0] = -learning_rate * grad_contact
        print('Action Movement:', self.contact_vel[0]*self.dt)

        return self.contact_vel[0] * self.dt
    ...
    def init_vel(self):
        ...
        # self.node_force[0][1] = 9.8 * self.node_mass * 50
...
def main():
    ...
    for i in range(100):
        ...
        # soft_obj.diff_data()
        error_tmp, L_tmp = soft_obj.construct_L()
        soft_obj.construct_dx_const()
        soft_obj.diff_pd(20)
        soft_obj.cal_ygrad()
        soft_obj.control_grasp(2.e0)
        # np.savetxt('rhs.csv', soft_obj.rhs.to_numpy(), delimiter=',', fmt='%.8f')
        # np.savetxt(f'grad_diffdata.csv', soft_obj.grad_diffdata.to_numpy(), delimiter=',', fmt='%.10f')
        print(f'Iter: {i}--------------------------------------')
        print(f'Error: {error_tmp}; Loss: {L_tmp}')
        print(f'Desired Node Position: {soft_obj.marker_pos_desired[0].to_numpy()}')
        print(f'Node Position: {soft_obj.node_pos[3].to_numpy()}')
        # print(f'dL dy: {soft_obj.dL_dy.to_numpy()}')
