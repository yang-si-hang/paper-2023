...
class SoftObject:
    def __init__(self, shape, seed_size):
        ...
        self.dL_feature = ti.Matrix.field(n=3, m=2 ,dtype=ti.f64, shape=())
    ...
    def construct_L(self)->float:
        ...
        for idx, i in ti.ndrange(3, 3):
            ...
            self.dL[self.triangle_points_idx[idx][i]*2] += self.feature_bary[idx][i] * self.dL_feature[None][idx, 0]
            self.dL[self.triangle_points_idx[idx][i]*2+1] += self.feature_bary[idx][i] * self.dL_feature[None][idx, 1]
