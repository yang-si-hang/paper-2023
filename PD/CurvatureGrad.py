...
feature_pos_init = np.array([[0.1 - curve_r, 0.05],
                             ...
                             [0.1, 0.05 - curve_r]])
feature_pos.from_numpy(feature_pos_init)
...
def kappa_grad():
    ...
    print('kappa_3 =', kappa_3)
    print('current_curvature =', current_curvature)
    print('Loss:', L)
...
if __name__ == '__main__':
    ...
    """
    # Check the gradient of kappa by difference method
    print('-----------------------------------------')
    U_np = np.array([[0., 1.],
                     [-1., 0.]])
    ...
    dkappa_2[1, :] = -kappa_2**3*f2.transpose()
    dkappa_2[2, :] = kappa_2**3*f2.transpose()
    dkappa_3[0, :] = f2.transpose()@U_np.transpose()
    dkappa_3[1, :] = f1.transpose()@U_np - f2.transpose()@U_np.transpose()
    dkappa_3[2, :] = -f1.transpose()@U_np
    ...
    delta_x = 1.e-8
    for i ,j in np.ndindex((3, 2)):
        feature_pos_tmp_np = feature_pos.copy()
