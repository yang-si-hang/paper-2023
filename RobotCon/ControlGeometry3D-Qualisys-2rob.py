...
class MyObject(SoftObject):
    ...
    def construct_L_soft(self, marker_pos_soft:npt.NDArray):
        ...
        # 计算角度余弦对每个点位置的梯度
        A, B, C = marker_pos_soft

        cos_u = np.dot(A-B, C-B)
        cos_v = np.linalg.norm(A-B) * np.linalg.norm(C-B)
        cos = cos_u / cos_v

        dcos_du = 1 / cos_u
        dcos_dv = -cos_u / cos_v**2

        du_dA = (C - B)
        du_dB = -(C - B) - (A - B)
        du_dC = A - B

        dv_dA = (A - B) * np.linalg.norm(C - B) / np.linalg.norm(A - B)
        dv_dB = - (A - B) * np.linalg.norm(C - B) / np.linalg.norm(A - B) - (C - B) * np.linalg.norm(
            A - B) / np.linalg.norm(C - B)
        dv_dC = (C - B) * np.linalg.norm(A - B) / np.linalg.norm(C - B)

        dcos_dA = dcos_du * du_dA + dcos_dv * dv_dA
        dcos_dB = dcos_du * du_dB + dcos_dv * dv_dB
        dcos_dC = dcos_du * du_dC + dcos_dv * dv_dC
        dcos_dmarker = np.vstack((dcos_dA, dcos_dB, dcos_dC))

        # 没有定义`desired loss`
        error_cos = cos - self.desired_loss
        for marker_i in range(POINTS_NUM):
            barycentric = self.barycentrics[marker_i]
            dloss_dmarker = 2 * error_cos * dcos_dmarker[marker_i, :]
            for idx, ele_idx in enumerate(self.marker_elements[marker_i]):
                self.dL_dq[ele_idx * dim] += dloss_dmarker[0] * barycentric[idx]
                self.dL_dq[ele_idx * dim + 1] += dloss_dmarker[1] * barycentric[idx]
                self.dL_dq[ele_idx * dim + 2] += dloss_dmarker[2] * barycentric[idx]

        return error_cos, np.linalg.norm(error_cos, axis=1) ** 2
